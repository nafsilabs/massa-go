...
func populateDatastore(contract ContractDatastore) ([]byte, error) {
	var datastore []ContractDatastoreEntry

	// number of contracts to deploy
	numberOfContractsKey := []byte{0}
	numberOfContracts := util.U64ToBytes(1)
	datastore = append(datastore, ContractDatastoreEntry{Key: numberOfContractsKey, Value: numberOfContracts})

	// contract bytecode
	datastore = append(datastore, ContractDatastoreEntry{Key: getContractByteCodeKey(), Value: contract.ByteCode})
	// args data
	// hardcoded for now, could be dynamix see: https://github.com/massalabs/massa-web3/blob/main/src/dataStore.ts
	datastore = append(datastore, ContractDatastoreEntry{Key: getArgsKey(), Value: contract.Args})
	...
	datastore = append(datastore, ContractDatastoreEntry{Key: getCoinsKey(), Value: util.U64ToBytes(contract.Coins)})
...
func DatastoreToDeployedContract(datastore []ContractDatastoreEntry) (
	...
	contract.Coins, err = util.BytesToU64(datastore[3].Value)
	if err != nil {
		return ContractDatastore{}, false
	}
...
func isContractDeployDatastore(datastore []ContractDatastoreEntry) bool {
	if len(datastore) == 4 &&
...
func DeserializeDatastore(datastore []byte) ([]ContractDatastoreEntry, error) {
	...
	var entries []ContractDatastoreEntry
	...
	for dataStoreIndex := uint64(0); dataStoreIndex < datastoreSize; dataStoreIndex++ {
		...
		entries = append(entries, ContractDatastoreEntry{Key: key, Value: value})
	}
